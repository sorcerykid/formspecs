--------------------------------------------------------
-- Minetest :: ActiveFormspecs Mod v2.8 (formspecs)
--
-- See README.txt for licensing and release notes.
-- Copyright (c) 2016-2020, Leslie E. Krause
--
-- ./games/minetest_game/mods/formspecs/init.lua
--------------------------------------------------------
...
local function recolor( text )
	return string.gsub( text, "@(#.-){(.-)}", function( color, input )
		return minetest.colorize( color, input )
	end )
end

local function escape( str, pat )
	return string.gsub( str, pat, function ( c )
		return string.format( "\\x%00X", string.byte( c ) )
	end )
end

local function unescape( str )
	return string.gsub( str, "\\x(..)", function ( b )
		return "\\" .. string.char( tonumber( b, 16 ) )
	end )
end

local function unescape_raw( str )
	return string.gsub( str, "\\x(..)", function ( b )
		return string.char( tonumber( b, 16 ) )
	end )
end
...
do
	...
	local rtime = clock( ) / 1000000

	afs.get_uptime = function( )
		return clock( ) / 1000000 - rtime
	end

	minetest.register_globalstep( function ( )
		local uptime = clock( ) / 1000000 - rtime
		local idx = #timers
		...
		while idx > 0 do
			...
			if uptime >= self.exptime then
				self.counter = self.counter + 1
				self.overrun = uptime - self.exptime
				self.exptime = uptime + self.form.timeout

				self.form.newtime = uptime
...
-----------------------------------------------------------------
-- define standard formspec variables and functions
-----------------------------------------------------------------

minetest.gui_bg = "bgcolor[#080808BB;true]"
minetest.gui_bg_img = "background[5.0,5.0;1.0,1.0;gui_formbg.png;true]"
minetest.gui_slots = "listcolors[#00000069;#5A5A5A;#141318;#30434C;#FFFFFF]"

minetest.get_hotbar_bg = function ( x, y )
	local str = ""
	for x_off = 0, 7 do
		str = out ..string.format( "image[%0.2f,%0.2f;1.0,1.0;gui_hb_bg.png]", x + x_off, y )
	end
	return str
end
...
minetest.get_context_field = function ( fields, prefix )
	for k, v in pairs( fields ) do
		if is_match( k, "^(" .. prefix .. "):(.+)$" ) then
			return _[ 2 ], _[ 1 ]
		end
	end
end
...
minetest.get_form_timer = function ( player_name, form_name )
	...
	self.start = function ( timeout )
		if not form.timeout and timeout >= 0.1 then
			local uptime = afs.get_uptime( )

			form.timeout = timeout
			table.insert( afs.timers, { form = form, counter = 0, oldtime = uptime, exptime = uptime + timeout, overrun = 0.0 } )
		end
	end
	...
	self.get_state = function ( )
		...
		for i, v in ipairs( afs.timers ) do
			...
			if v.form == form then
				return { elapsed = uptime - v.oldtime, remain = v.exptime - uptime, overrun = v.overrun, counter = v.counter }
			end
		end
	end
...
-----------------------------------------------------------------
-- construct helper object to manage form groups
-----------------------------------------------------------------

local function create_groups( formspec )
	local self = { }
	local groups = { }

	local function tokenize( str )
		str = escape( str, "[{}]" )
		str = string.gsub( str, "group%[(.-)%]", "%1:{" )
		str = string.gsub( str, "group_end%[%]", "}" )
		return str
	end

	local function insert_raw( name, text )
		text = string.gsub( text, "([a-zA-Z_]+):(%b{})", function ( subname, subtext )
			insert_raw( subname, string.sub( subtext, 2, -2 ) )
			return "group[" .. subname .. "]"
		end )
		groups[ name ] = text
	end

	self.insert = function ( name, formspec )
		insert_raw( name, tokenize( formspec ) )
	end

	self.update = function ( name, formspec )
		for subname in string.gmatch( groups[ name ], "group%[(.-)%]" ) do
			self.update( subname )
		end
		if formspec then
			self.insert( name, formspec )
		else
			groups[ name ] = nil
		end
	end

	self.compile = function ( name )
		local text = string.gsub( groups[ name or "root" ], "group%[(.-)%]", function ( subname )
			return self.compile( subname )
		end )
		return unescape_raw( text )
	end

	self.insert( "root", formspec )
...
minetest.create_form = function ( meta, player_name, formspec, on_close, signal )
	...
	form.meta = meta or { }
	form.oldtime = afs.get_uptime( )
	form.newtime = form.oldtime
	form.groups = create_groups( formspec )

	afs.forms[ player_name ] = form
	afs.stats:on_open( )
	minetest.show_formspec( player_name, form.name, parse_elements( form, form.groups.compile( ) ) )

	return form.name
end

minetest.update_form = function ( player, formspec )
	local player_name = type( player ) == "string" and player or player:get_player_name( )
	local form = afs.forms[ player_name ]
	...
	if form then
		...
		if type( formspec ) == "table" then
			for subname, subtext in pairs( formspec ) do
				form.groups.update( subname, subtext )
			end
			minetest.show_formspec( player_name, form.name, parse_elements( form, form.groups.compile( ) ) )
		else
			form.groups.insert( "root", parse_elements( form, formspec ) )
...
minetest.destroy_form = function ( player, signal )
	...
	if form then
		minetest.close_formspec( player_name, form.name )
		minetest.get_form_timer( player_name ):stop( )
...
minetest.register_chatcommand( "fs", {
	...
	func = function( player_name, param )
		...
		local function get_sorted_forms( )
			local f = { }
		...
		local function get_formspec_groups( )
			local uptime = afs.get_uptime( )
			local groups = {
				uptime = string.format( "label[0.1,0.5;%d min %02d sec]", math.floor( uptime / 60 ), uptime % 60 ),
				active = string.format( "label[5.6,0.5;%d]", afs.stats.active ),
				opened = string.format( "label[6.9,0.5;%d]", afs.stats.opened ),
				closed = string.format( "label[8.2,0.5;%d]", afs.stats.closed ),
				listing = "",
			}
			...
			for idx = ( page_idx - 1 ) * page_size + 1, math.min( page_idx * page_size, #sorted_forms ) do
				...
				groups.listing = groups.listing
					.. string.format( "image_button[0.0,%0.1f;0.6,0.6;;del:%s;x]", vert, player_name )
		...
		local get_formspec = function ( )
			local groups = get_formspec_groups( )

			local formspec = "size[9.5,7.3]"
				.. minetest.gui_bg
				.. minetest.gui_bg_img

				.. "label[0.1,6.7;ActiveFormspecs v2.8]"
				.. recolor( "label[0.1,0.0;@#888888{uptime:}]group[uptime]" .. groups.uptime .. "group_end[]" )
				.. recolor( "label[5.6,0.0;@#888888{active:}]group[active]" .. groups.active .. "group_end[]" )
				.. recolor( "label[6.9,0.0;@#888888{opened:}]group[opened]" .. groups.opened .. "group_end[]" )
				.. recolor( "label[8.2,0.0;@#888888{closed:}]group[closed]" .. groups.closed .. "group_end[]" )
				...
				.. "group[listing]" .. groups.listing .. "group_end[]"
		...
		local on_close = function( meta, player, fields )
			if fields.quit == minetest.FORMSPEC_SIGTIME then
				...
				minetest.update_form( player_name, get_formspec_groups( ) )
		...
		minetest.get_form_timer( player_name ).start( 1.0 )
